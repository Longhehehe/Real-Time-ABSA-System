...
def predict_multipolarity(
    ...
    with torch.no_grad():
        for text in texts:
            ...
            # Predict
            pred_m = (torch.sigmoid(logits_m) > 0.5).int().cpu().numpy()[0]
            pred_s = (torch.sigmoid(logits_s) > 0.5).int().cpu().numpy()[0]
            # pred_s shape: [aspects, 3]
            
            res = {}
            for idx, aspect in enumerate(ASPECTS):
                is_mentioned = pred_m[idx] == 1
                
                # Get sentiments
                sentiments = []
                if is_mentioned:
                    s_vec = pred_s[idx] # [NEG, POS, NEU]
                    for s_idx, is_active in enumerate(s_vec):
                        if is_active:
                            sentiments.append(sentiment_names[s_idx])
                    
                    # Fallback if mentioned but no sentiment predicted (should rarely happen)
                    if not sentiments:
...
def evaluate_model(
    ...
    device: str = None,
    batch_size: int = 16,
    sample_limit: int = 200  # Default limit to 200 for faster UI feedback
) -> Dict:
    """
    Evaluate model on validation set (split from data_path).
    Returns metrics dict.
    """
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    
    print(f"🚀 Evaluating PhoBERT Multi-Polarity Model (Limit: {sample_limit})")
    print(f"📱 Device: {device}")
    ...
    # Split to get SAME validation set as training (random_state=42)
    _, val_texts, _, val_labels_m, _, val_labels_s = train_test_split(
        texts, labels_m, labels_s, test_size=0.2, random_state=42
    )
    
    # Apply limit
    if sample_limit and len(val_texts) > sample_limit:
        print(f"   ⚠️ Limiting validation set from {len(val_texts)} to {sample_limit} samples for speed")
        # Use random indices to get diverse samples
        indices = np.random.choice(len(val_texts), sample_limit, replace=False)
        val_texts = [val_texts[i] for i in indices]
        val_labels_m = val_labels_m[indices]
        val_labels_s = val_labels_s[indices]
    ...
    if mentioned_mask.sum() > 0:
        ...
        pred_s_mentioned = pred_s_flat[mentioned_mask]
        f1_s = f1_score(true_s_mentioned, pred_s_mentioned, average='samples', zero_division=0)
        f1_s_micro = f1_score(true_s_mentioned.flatten(), pred_s_mentioned.flatten(), average='binary', zero_division=0)
    else:
        f1_s = 0.0
        f1_s_micro = 0.0
    ...
    results = {
        "samples": len(val_texts),
        "mention_f1": float(f1_m),
        "sentiment_f1": float(f1_s),
        "sentiment_f1_micro": float(f1_s_micro),
        "combined_f1": float(combined_f1),
        "best_training_f1": float(checkpoint.get('best_f1', 0.0))
    }
...
if __name__ == "__main__":
    ...
    # Use ALL labeled data from labeled/ folder
    DATA_PATH = os.path.join(PROJECT_DIR, 'labeled')
    OUTPUT_DIR = os.path.join(PROJECT_DIR, 'models', 'phobert_absa_multipolarity')
