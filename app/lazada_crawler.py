...
def crawl_reviews(
    ...
    while len(all_reviews) < max_reviews:
        ...
        max_crawl_retries = 3
        for attempt in range(max_crawl_retries):
            try:
                print(f"  ⏳ Requesting Page {page} (Attempt {attempt+1})...")
                response = session.get(api_url, params=params, timeout=20) # Reduced timeout to 20s
                print(f"  📥 Got response: Length {len(response.content)} bytes")
        ...
        # Success path (inside while, after break from for-loop)
        try:
            data = response.json()
            ...
            if not items:
                error_hint = ""
                if "punish" in response.text or "rgv587_flag" in response.text:
                    error_hint = "🤖 LAZADA BOT DETECTED! (Cần refresh cookies mới + giải Captcha trên trình duyệt)"
                elif "login" in response.text:
                     error_hint = "🔒 LOGIN REQUIRED! (Cookies hết hạn)"
                
                print(f"⚠️ Response content (first 500 chars): {response.text[:500]}")
                if error_hint:
                    print(f"❌ {error_hint}")
                    return [], error_hint # Return immediately with error hint
