"""Independently validate the corrected Q1 human-gold plan v1.1."""
...
def validate(plan_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "q1-human-gold-sampling-plan-manifest/1.1.0"
        or report.get("schema_version")
        != "q1-human-gold-sampling-plan/1.1.0"
        ...
        raise ValueError("Sampling plan v1.1 status/type/schema mismatch")
    if (
        ...
        or config["supersedes_plan_id"]
        != "q1-human-gold-plan-a7db74c454ac54b0"
    ):
        raise ValueError("Sampling plan source/config/supersession mismatch")
    ...
    capacity = report["group_aware_capacity"]
    if (
        ...
        or capacity["selenium_eligible_groups"] != 9
        or capacity["selenium_eligible_records"] != 231
        or capacity["selenium_core_capacity_upper_bound"] != 62
        or capacity["selenium_required"] != 50
        or capacity["headroom"] != 12
    ):
        raise ValueError("Challenge/target/group-aware capacity mismatch")
    eligibility = report["eligibility"]
    if (
        eligibility["eligible_records"] != 15_553
    ...
    return {
        ...
        "selenium_group_aware_capacity": capacity[
            "selenium_core_capacity_upper_bound"
        ],
        "selenium_required": capacity["selenium_required"],
